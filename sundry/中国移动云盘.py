...
#     - 1、抓包地址：https://yun.139.com/m/#/login
#     - 2、三网号码登录移动云盘， 登陆后找到浏览器工具箱点击开发者工具，login 接口 resp 中找到 token，任意接口 header 找到 Authorization
# 环境变量设置:
...
ua = 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MCloudApp/10.5.4 iPhone AppLanguage/zh-CN'
    ...
    def do_fruit_task(self, task_name, task_id, water_num):
        ...
        if do_task_data.get('success'):
            ...
            if get_water_data.get('success'):
                ...
                print(f'❌领取失败: {get_water_data.get("msg", "未知错误")}')
        else:
            print(f'❌参与任务失败: {do_task_data.get("msg", "未知错误")}')
