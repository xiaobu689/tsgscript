...
class Task {
    ...
    async user_info() {
        ...
        if (result.code == 0) {
            $.log(`[${result.data.nickName}]积分[${result.data.integralCount}]`)
            this.ckStatus = true
        } else {
            this.ckStatus = false
            $.log(result)
    ...
    async task_share(id) {
        ...
        result.code == 0 ? (result.data.result == true ? $.log(`分享成功`) : $.log(`分享失败`)) : $.log(`分享失败`)
    }
    async task_like(id) {
        ...
        result.code == 0 ? (result.data.result == true ? $.log(`点赞成功`) : $.log(`点赞失败`)) : $.log(`点赞失败`)
    }
}
