...
class SHPT():
    ...
    def sign(self):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/personal/score/sign'
    ...
    def total_score(self):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/personal/score/total'
        response = make_request(url, json_data, 'post', self.headers)
        if response and response['code'] == 0:
    ...
    def today_score(self):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/personal/score/info'
        response = make_request(url, json_data, 'post', self.headers)
        if response and response['code'] == 0:
    ...
    def article_list(self):
        channels = ["976a0e207f574d8783c45b0c9d67628f", "1170902d3d6442c99373b082b844985a", "af483e4cc0f5478e93467f45f5fab6bc", "af483e4cc0f5478e93467f45f5fab6bc", "c6c7d13ff92f4ac99d25cb6ad09962e3"]
    ...
    def article_read_points_add(self):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/points/read/add'
    ...
    def article_count_usage_desc(self, id):
        ...
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/common/count/usage/inc'
        make_request(url, json_data, 'post', self.headers)

    def article_content(self, id):
        json_data = {'id': id}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/news/content/get'
        response = make_request(url, json_data, 'post', self.headers)
        return response
    ...
    def article_favor(self, id):
        json_data = {'id': id}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/news/content/favor'
        response = make_request(url, json_data, 'post', self.headers)
        if response and response['code'] == 0:
            print(f'✅文章收藏成功')
        else:
    ...
    def article_share(self, id):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/points/share/add'
        response = make_request(url, json_data, 'post', self.headers)
        if response and response['code'] == 0:
            print(f'✅文章分享成功')
        else:
    ...
    def video_view_task(self):
        json_data = {}
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/points/video/add'
    ...
    def article_comment_add(self, id, content):
        ...
        url = 'https://ptweb.shmedia.tech/media-basic-port/api/app/common/comment/add'
...
if __name__ == '__main__':
    ...
    print(f"上海普陀共获取到{len(tokens)}个账号")
    for i, account_info in enumerate(tokens, start=1):
        print(f"\n======== ▷ 第 {i} 个账号 ◁ ========")
        SHPT(account_info).main()
        print("\n随机等待30-60s进行下一个账号")
