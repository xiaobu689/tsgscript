...
_________________Code.K修改 2024.2.27______________________

环境变量: 名称：bd_mttoken   值：美团token的值#uuid的值 抓包app或者网页的token=Agxxxx 只要token后面的值 UUID在APP抓
环境变量: 名称：mtgurl       值：code.K 的 H5 mtgsig 服务接口地址
环境变量: 名称：mtgurltoken  值：mtmtgsig 服务接口地址的授权Token

修改来着彼得2024.1.11发的源码,对接mtgsig接口和fingerprint接口，所以只需要3个环境变量就能跑了。
自动判断是否需要mtgsig，白号只需要获取一次指纹就好~需要打印日记 环境变量 bd_isprint 填 True
...
如果没推送，大概的环境变量格式就是
AgXXXXXXXXXX#00000000XXXXXXX#d41bgtHhXXXXXXXXX
有推送就是
AgXXXXXXXXXX#00000000XXXXXXX#d41bgtHhXXXXXXXXX#UID_XXXXXXX 
多账号默认新建变量或者用 & 分开
...
# mtgsig接口地址变量：
mtgurl = os.environ.get("mtgurl")
if mtgurl is None:
    mtgurl = ''

# mtgsig接口token变量：
mtgurltoken = os.environ.get("mtgurltoken")
if mtgurltoken is None:
    mtgurltoken = ''
...
# 调试设置
########################################################
# 是否打印详细日志 True/False  默认True 改False 为关
...
# 如果没跑成功，应该是你的指纹的不对，
# 默认第二遍最大出现还是有超过30个任务就说明指纹有异常就不跑了，请进APP手动领取试试
# 能手动领说明账号正常，抓的指纹有问题。这个自己上安卓虚拟机或者真实安卓机上去抓
# 我没用IOS，不清楚IOS的指纹是怎么样的，但如果你用IOS，可以去抓安卓机的指纹尝试！
# 如果指纹不对会导致一直循环去领取，这样会导致频繁访问mtgsig接口，消耗积分！导致归0！请注意！
# 请保证你传的指纹参数是正确，否则这个参数请不要随意改动
# 
...
def Get_fingerprint(UUID,Token): 
    try_count = 3
    while try_count > 0:
        try:
            url = 'https://mtg.codek.cloudns.org/api/fingerprint' #填mtgsig API地址
            data = {
                "Token":Token, #获取fingerprint接口地址授权的Token TG机器人 @Code_K_Bot 获取 
                "UUID":UUID #美团UUID,请对应CK
            }
            res=requests.post(url, json=data) #将数据提交给mtgsig服务器处理
            if res.json()['Code'] == 200:
                points=res.json()['points']
                print(f'fingerprint接口调用成功，剩余{points}积分') #打印出还有多少积分
                fingerprint=res.json()['fingerprint']
                return(fingerprint)
            else:
                print(res.text) #返回报错情况 格式不对 积分不足 等情况
        except Exception as e:
                try_count -= 1
                print(f'获取接口fingerprint异常{e},还有{try_count}次重试机会')
                if try_count == 0:
                    print(f'无法访问获取fingerprint服务器，结束运行~')
                    exit(0)
...
class Mttb:
    def __init__(self, zh, ck):
        ...
        if "UID" in ck:
            ...
            self.uuid = ck.split('#')[1]
            self.uid = None
        self.fingerprint = Get_fingerprint(self.uuid,mtgurltoken)
        self.ddmsg = None
        self.login = None
        self.lisss = None
        ...
        self.gglid = None
        self.cardId = None
        self.yanzhenmtg = False

    def MTlogin(self):
        try:
            url = "https://open.meituan.com/user/v1/info/auditting?fields=auditAvatarUrl%2CauditUsername%2CauditBio"
            h = {
            ...
            if 'username' in r.text:
                ...
                print(f'👤账号[{self.num}] 🆔{self.name}')
                xx = f'👤账号[{self.num}] 🆔{self.name}\n'
                self.startmsg += xx
                return True
            else:
                print(f'👤账号[{self.num}]登录异常，请检查CK',r.json())
                return False
        except Exception as e:
            print(f'👤账号[{self.num}]登录异常：{e}')
            return False




    def Getmtgsig_data(self,urls,headers,datas):
        try_count = 3
        while try_count > 0:
            try:
                if mtgurl == '':
                    print('⛔mtgurl变量为空，结束运行')
                    exit(0)
                if mtgurltoken == '':
                    print('⛔mtgurltoken变量为空，结束运行')
                    exit(0)
                url = mtgurl #mtgsig API地址
                data = {
                    "Token": mtgurltoken, #mtgsig接口授权Token
                    "jsondata":{
                        "url": urls, #美团请求的URL 不用动
                        "data": datas #美团请求的data 不用动
                    } 
                }

                res=requests.post(url, json=data) #将数据提交给mtgsig服务器处理
                #print(url,data,res.text)
                MtgData = res.json()['MtgData'] #mtgsig服务器处理后返回的数据
                url = MtgData['url'] #从数据中取出 已经带好mtgsig的URL
                data = MtgData['data'] #从数据中取出 处理好的 美团请求的data数据
                points = res.json()['points'] #查询Token积分
                data=json.dumps(data, separators=(',', ':')) #压缩数据去空格
                if isprint:
                    self.startmsg += f'🆔账号{self.num}-{self.name} mtgsig接口调用成功，剩余{points}积分\n'
                if isdl:
                    r = requests.post(url=url, headers=headers, data=data, timeout=10, verify=False, proxies=proxy)
                else:
                ...
                try_count -= 1
                print(f'👤账号[{self.num}]获取接口MtgData异常{e},还有{try_count}次重试机会')
                if try_count == 0:
                    print(f'👤账号[{self.num}]无法访问获取mtgsig服务器，结束运行~')
                    exit(0)
    ...
    def get_ids(self):
        ...
        while try_count > 0:
            try:
                ...
                if self.yanzhenmtg == True:
                    r = self.Getmtgsig_data(url,h,data)
                else:
                    if isdl:
                        r = requests.post(url, headers=h, json=data, timeout=10, verify=False, proxies=proxy)
                    else:
                ...
                if r.status_code == 403:
                    print(f'🆔账号{self.num}-{self.name} 获取任务列表403，需要验证mtgsig,尝试切换第三方获取mtgsig接口访问')
                    self.yanzhenmtg = True
                try_count -= 1
                time.sleep(random.randint(2, 5))
                continue
        ...
        while try_count > 0:
            try:
                ...
                if r.status_code == 200:
                    ...
                    if isprint:
                        self.endmsg += f'🆔账号{self.num}-{self.name} {self.id}完成任务失败：还有{try_count - 1}次重试机会\n'
                    try_count -= 1
                    time.sleep(random.randint(3, 8))
                    continue
            except:
    ...
    def post_id(self):
        ...
        while try_count > 0:
            try:
                ...
                if self.yanzhenmtg == True:
                    r = self.Getmtgsig_data('https://game.meituan.com/mgc/gamecenter/front/api/v1/mgcUser/task/receiveMgcTaskReward?yodaReady=h5&csecplatform=4&csecversion=2.3.1',
                        headers,data)
                else:
                    if isdl:
                        r = requests.post(
                            ...
                            headers=headers, json=data, timeout=10, verify=False, proxies=proxy)
                    else:
                        r = requests.post(
                            'https://game.meituan.com/mgc/gamecenter/front/api/v1/mgcUser/task/receiveMgcTaskReward?yodaReady=h5&csecplatform=4&csecversion=2.3.1',
                            headers=headers, json=data, timeout=10, verify=False)
                if isprint:
                    print(f'🆔账号{self.num}-{self.name}({self.usid}) 领取任务奖励{self.id} {r.json()}')
                if r.status_code == 200:
                    if r.json()['msg'] == 'ok':
                        time.sleep(random.randint(1, 3))
                        return True
                    ...
                    if isprint:
                        self.endmsg += f'🆔账号{self.num}-{self.name} {self.id}领取任务奖励异常：还有{try_count - 1}次重试机会\n'
                    try_count -= 1
                    time.sleep(random.randint(3, 8))
...
if __name__ == '__main__':
    ...
    msg = "🔔当前版本V1.11(手动抓包APP的指纹+调用mtgsig接口)\n源码作者：彼得\n修改：Code.K \n2024.2.9更新日志:\n1、移除彼得服务器授权\n2、添加环境变量: 名称：mtgurl mtgurltoken  Fingerprint 具体看文件最头部的注释！\n3、自动判断账号是否验证mtgsig，需要则自动切换使用接口\n4、更多变量看注释！！！有能力自己改本地指纹算法"
    print(msg)
