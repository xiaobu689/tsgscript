...
class UserInfo {
    ...
    async video_like() {
        try {
            ...
            for (let index = 0; index < 5; index++) {
                ...
                const data = { "type": 1, "video_id": video_id }
                let options = {
    ...
    async member_follow() {
        try {
            let user_id = ""
            for (let index = 0; index < 5; index++) {
                user_id = this.videoList[index].detail.user_id
                const data = { "type": 1, "user_id": user_id }
                let options = {
                ...
                //console.log(options);
                if (result.message == "success！") {
                    $.DoubleLog(`✅账号[${this.index}]  关注用户成功🎉`);

                } else {
                    $.DoubleLog(`❌账号[${this.index}]  关注用户失败`);
                    //console.log(result);
                }
            }
    ...
    async task_list() {
        try {
            ...
            if (result.message == "success！") {
                ...
                if (result.data["5"].step["1"].finish_progress < result.data["5"].step["1"].condition) {
                ...
                if (result.data["3"].step["1"].finish_progress < result.data["3"].step["1"].condition) {
                    $.DoubleLog(`✅账号[${this.index}]  执行关注用户任务🎉`)
                    await this.member_follow()
                } else if (result.data["3"].step["1"].com_status == 12) {
                    await this.receive_coin(3)
                }
                //5评论
                //6发布
                //3关注
